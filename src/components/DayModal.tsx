...
export function DayModal({ item, contentUrl, onClose }: { item: CalendarDay; contentUrl?: string; onClose: () => void }) {
  ...
  return (
    <div className="backdrop-enter fixed inset-0 z-50 flex items-end justify-center bg-[#3b2429]/45 p-0 backdrop-blur-sm sm:items-center sm:p-5" onMouseDown={onClose}>
      <div role="dialog" aria-modal="true" aria-labelledby="day-title" onMouseDown={(event) => event.stopPropagation()} className="modal-enter max-h-[94vh] w-full max-w-2xl overflow-y-auto rounded-t-[2rem] bg-[#fffaf6] p-5 shadow-2xl sm:rounded-[2.25rem] sm:p-8">
        <div className="flex items-start justify-between gap-5">
          ...
          <button type="button" onClick={onClose} aria-label="Cerrar sorpresa" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#f4e4df] text-[#805e62] transition hover:bg-[#edd3cf]">
            <CloseIcon />
          </button>
        </div>
        <div className="mt-7">
